...
export default function DashboardClient() {
  ...
  return (
    <div className="min-h-screen flex flex-col t-bg t-text bg-dot-pattern selection:bg-[#00F0FF]/20 selection:text-[#00F0FF] transition-layout">
        ...
        {selectedAddress && (
          <section className="w-full max-w-[1600px] mx-auto flex flex-col gap-6 animate-fadeIn">
            ...
            {aiNotice && aiNotice.code === 'ZERO_POSITIONS' && (
              <div className="w-full panel-architecture bg-[var(--cp-surface)] p-8 border-t-2 border-t-cyan-500 font-mono flex flex-col gap-6">
                <div className="flex items-center justify-between border-b t-border pb-4">
                  <div className="flex items-center gap-3">
                    <span className="h-2.5 w-2.5 bg-cyan-500"></span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                  <div className="p-3 bg-[var(--cp-surface-elevated)] border t-border flex flex-col gap-1">
                    <span className="text-[9px] t-text-muted uppercase font-bold tracking-wider">NETWORK SCANNED</span>
                    <span className="text-xs font-bold t-text">Ethereum Mainnet (Chain ID 1)</span>
                  </div>
                  <div className="p-3 bg-[var(--cp-surface-elevated)] border t-border flex flex-col gap-1">
            ...
            {aiAnalysis && (
              <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start mt-4">
                ...
                <div className="xl:col-span-3 flex flex-col gap-4">
                  <div className="flex items-center justify-between border-b t-border pb-2 font-mono">
                    <h4 className="text-xs font-bold uppercase tracking-widest t-text-secondary">
                      INDEXED POSITIONS
                    </h4>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 mt-2 overflow-y-auto max-h-[600px] pr-2 custom-scrollbar">
                      ...
                      return (
                        <div
                          ...
                          className={`p-3 font-mono cursor-pointer transition-all border-l-2 ${
                            isFocused 
                              ? 'bg-[var(--cp-surface-elevated)] border-l-cyan-500 border-y border-r t-border shadow-sm' 
                              : 'bg-[var(--cp-surface)] border-l-transparent border-y border-r border-y-transparent border-r-transparent hover:bg-[var(--cp-surface-elevated)] hover:t-border t-text-secondary'
                          }`}
                        >
                          <div className="flex items-center justify-between mb-2">
                            <span className={`font-bold text-xs tracking-tight ${isFocused ? 't-text-heading' : 't-text'}`}>
                              {pos.tokenPair}
                            </span>
                            <span className="text-[9px] bg-[var(--cp-bg)] px-1.5 py-0.5 t-text-muted border t-border">#{pos.positionId}</span>
                          </div>
                    ...
                    {filteredPositions.length === 0 && !aiLoading && (
                      <div className="text-xs t-text-muted font-mono italic p-4 text-center border border-dashed t-border">
                  ...
                  <div className="mt-6 panel-architecture p-4 font-mono flex flex-col gap-3 bg-[var(--cp-surface)]">
                    <div className="text-[10px] font-bold uppercase tracking-widest text-[#baf24a] border-b t-border pb-2">
                      SYSTEM TELEMETRY STATUS
                    </div>
                    <div className="flex flex-col gap-2 text-[10px] t-text-secondary">
                      <div className="flex justify-between items-center bg-[var(--cp-surface-elevated)] p-1.5 border t-border">
      ...
      <footer className="border-t t-border-strong bg-[var(--cp-bg)] py-4 px-6 font-mono text-[10px] t-text-muted flex flex-wrap items-center justify-between gap-4 uppercase font-bold tracking-widest z-50 relative">
        <div className="flex items-center gap-6">
          <span className="t-text font-black">CHAINPILOT</span>
          <span className="t-text-muted">|</span>
